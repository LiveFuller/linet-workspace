-- LINET Workspace — Supabase one-shot installer (setup.sql)
-- Canonical source: supabase/migrations/*.sql — this file is GENERATED by concatenating migrations.
-- This file matches supabase/migrations/0001_init.sql exactly (functional SQL below is byte-identical).
-- Do NOT edit setup.sql independently; edit migrations and regenerate.
...
COMMIT;
-- Waygo engine â€” hotel QR slugs, outreach, scans, bookings
BEGIN;
...
CREATE TABLE IF NOT EXISTS public.waygo_hotels (
  ...
  area text NOT NULL DEFAULT 'Praha 1' CHECK (area IN ('Praha 1','Praha 2','Praha 3','Praha 4','Praha 5','Praha 6','Praha 7','Praha 8','CelĂˇ Praha')),
...
-- public read for slug resolution (anon can resolve QR to hotel? needed for redirect) â€” allow anon SELECT on slug lookup via function, but not direct table anon select
